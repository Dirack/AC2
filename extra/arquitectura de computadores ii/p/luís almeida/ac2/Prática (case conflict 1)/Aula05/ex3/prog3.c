...
int main(void){
  ...
  while(1){
    ...
    int tmp = readCoreTimer();    // para guardar o tempo de impressão

    LATEbits.LATE0 = 0;           // desligar LED
    printInt(ADC1BUF0, 16);
    printStr(" ");
    printInt(tmp, 10);            // impressao do tempo de conversao 
    
    //(NESTE caso deu tmp = 73)
