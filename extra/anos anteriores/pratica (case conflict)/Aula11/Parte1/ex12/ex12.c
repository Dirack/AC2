...
void main(void)
{
	...
	for(;;)
	{
		printf("Insira um caracter(R/W):\n");
		char c = getChar();
		if( c == 'R'){
			printf("Insira um endereço:\n");
			addr = readInt(16);
			printf("\nO endereço %d tem o valor ",addr);
			printInt(eeprom_readData(addr),10);
			printf("\n");
		}
		else if(c == 'W'){
			printf("Insira um endereço:\n");
			...
			addrtmp = addr;
			for(i = 0; i < 16; i++){
				eeprom_writeData(addrtmp,val);
				addrtmp++; val++;
			}
			printf("\nendereço inicial %d:",addr);
			addrtmp = addr;
			for(i = 0; i < 16; i++){
				printInt(eeprom_readData(addrtmp),10);
				printf(" ");
				addrtmp++;	
			}
			printf("\n");
		}
		else printf("Caracter inválido! \n");
	}
}
